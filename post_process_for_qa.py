...
#             用于选择无效答案的阈值：如果最佳答案的得分小于无效答案的得分减去此阈值，
#             则为该示例选择无效答案（请注意，该示例为无效答案的得分 给出几个特征是每个特征上空答案的最低分数：
#             所有特征都必须根据它们“想要”来预测空答案的事实而对齐）
#     """
...
#             no_answer_probs[example['id']] = answerable_probs[0]
#
...
#         } for pred in predictions]
#
#     return all_predictions, no_answer_probs
import collections
import numpy as np
...
def postprocess_qa_predictions(examples,
    ...
    # print('\n')
    # print('start:', all_start_logits.shape)   #(len,256)
    # print('cls', all_cls_logits.shape)        #(len,2)
    # Build a map example to its corresponding features.
    features_per_example = collections.defaultdict(list)
    for i, feature in enumerate(features):
        features_per_example[feature["example_id"]].append(i)
    ...
    for example_index, example in enumerate(examples):
        # Those are the indices of the features associated to the current example.
        feature_indices = features_per_example[example['id']]   # [0,1,2]
